  ...
  if params[:search_type].to_s == 'password'
    ...
    if @local_results.nil? || @local_results.empty?
      ...
      @local_results.each do |local_entry|
        ...
        results_entry['hub_cracked'] = '0' # Because we're not querying the hub (but maybe we should)
      end
    end
    ...
    @results = repository(:default).adapter.select("SELECT a.username, h.id, h.plaintext, h.originalhash, h.hashtype, c.name FROM hashes h LEFT JOIN hashfilehashes a on h.id = a.hash_id LEFT JOIN hashfiles f on a.hashfile_id = f.id LEFT JOIN customers c ON f.customer_id = c.id WHERE a.username like '%" + params[:value] + "%'")
