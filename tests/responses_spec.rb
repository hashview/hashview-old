...
class MyTest < MiniTest::Unit::TestCase
  ...
  def delete_all_customers
    puts 'Deleteing all customer'
    Customers.all.each(&:delete)
  end

  # Logs in a single test user
  def login_testuser
    delete_all_users
    @user = build(:user, username: 'test', password: 'Omgpl@in5', admin: true)
  ...
  def test_register_post
    ...
    delete_all_users
    post '/register', {username: 'reg_test', password: 'Omgpl@in5', confirm: 'Omgpl@in5'}
    assert last_response.redirection?
    assert_equal "http://example.org/login", last_response.location
    follow_redirect!
    assert last_response.ok?
  ...
  def test_customers_create_post
    ...
    post '/customers/create', {name: customer_name, desc: 'cust_test_desc'}
    assert last_response.redirection?
  ...
  def test_invalid_email_send_response
    ...
    user.update(email: '', password: 'Omgpl@in5')
    get '/test/email'
    assert last_response.redirection?
